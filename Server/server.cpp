...
void db_messageSend() {
    ...
    pstmt = con->prepareStatement("INSERT INTO message_room (number, user_id, content, time, chatroom_num) values(null,?,?,now(),?)"); // INSERT

    pstmt->setString(1, tokens[1]); // 보낸 사람 아이디
    pstmt->setString(2, tokens[2]); // 보낸 메세지
    pstmt->setInt(3, 1); //챗 룸 넘버 고쳐야 함
...
void db_selectQuery_ver2() {
    ...
    while (res->next()) {
        ...
        row.push_back(res->getString("chatroom_num"));
    ...
    //cout << "\n\n" << endl;
    //int a = result.size();
    //const std::vector<std::string>& row = result[3];
    //for (const std::string& value : row) {
    //    std::cout << value << " ";
    //}
    //cout << "\n" << endl;
